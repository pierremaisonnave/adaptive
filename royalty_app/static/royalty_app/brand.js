...
function save(contract_id){
    ...
    for (var i = 0; i < length_table; i++) {
        brand_id=tr_list[i].children[0].children[0].value
        if (brand_id == "" ){brand_id=0 } // id is an int, so we need to change all "" value to smth, here we choose 0
        m3_brand_code=tr_list[i].children[1].children[0].value
        brand_name=tr_list[i].children[2].children[0].value
        import_array.push(`{"brand_id":"${brand_id}","m3_brand_code":"${m3_brand_code}","brand_name":"${brand_name}"}`)
    } 
