...
function save(contract_id){
    ...
    for (var i = 0; i < length_table; i++) {
        sales_breakdown_item_id=tr_list[i].children[0].children[0].value
        if (sales_breakdown_item_id == "" ){sales_breakdown_item_id=0 } // id is an int, so we need to change all "" value to smth, here we choose 0
        sales_breakdown_definition=tr_list[i].children[1].children[0].value
        import_array.push(`{"sales_breakdown_item_id":"${sales_breakdown_item_id}","sales_breakdown_definition":"${sales_breakdown_definition}"}`)
    } 
