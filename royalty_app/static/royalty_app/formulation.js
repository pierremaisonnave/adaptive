...
function save(contract_id){
    ...
    for (var i = 0; i < length_table; i++) {
        ...
        formula_code= tr_list[i].children[0].children[0].value
        formula_name= tr_list[i].children[1].children[0].value

        import_array.push(`{"formula_code":"${formula_code}","formula_name":"${formula_name}"}`)
    } 

    import_string="["+import_array+"]"

    // We load it via a fetch in the API
    fetch(`/save_formulation`, {
        method: 'POST',
        body: import_string
        })
...
function is_any_fied_empty(){
    ...
    for (var i = 0; i < length_table; i++) {
        if (tr_list[i].children[0].children[0].value=="" || tr_list[i].children[1].children[0].value=="" ){
            return true
        }
    } 
    return false
